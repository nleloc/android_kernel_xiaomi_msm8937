 ...
 *
 * $Revision: 23175 $
 * $Date: 2018-02-12 16:26:21 +0800 (週一, 12 二月 2018) $
 *
...
const uint16_t gesture_key_array[] = {
	KEY_GESTURE_C,  //GESTURE_WORD_C
	KEY_GESTURE_W,  //GESTURE_WORD_W
	KEY_GESTURE_V,  //GESTURE_WORD_V
	KEY_WAKEUP,  //GESTURE_DOUBLE_CLICK
	KEY_GESTURE_Z,  //GESTURE_WORD_Z
	KEY_GESTURE_M,  //GESTURE_WORD_M
	KEY_GESTURE_O,  //GESTURE_WORD_O
	KEY_GESTURE_E,  //GESTURE_WORD_e
	KEY_GESTURE_S,  //GESTURE_WORD_S
	KEY_GESTURE_UP,  //GESTURE_SLIDE_UP
	KEY_GESTURE_DOWN,  //GESTURE_SLIDE_DOWN
	KEY_GESTURE_LEFT,  //GESTURE_SLIDE_LEFT
	KEY_GESTURE_RIGHT,  //GESTURE_SLIDE_RIGHT
};
#endif
...
info_retry:
	...
	NVT_LOG("HQ fw_ver=%x \n",ts->fw_ver);
	#if defined(CONFIG_GET_HARDWARE_INFO)
	if(ts->nvt_pid ==0x6205)
		gtp_module_id = 0x13;
	sprintf(tpdname,"CTPFW_NT36572A_HX_%02x.%02x",gtp_module_id,ts->fw_ver);
	register_hardware_info(CTP, tpdname);
        #endif
...
int GESTURE_ENABLE = 1;
static ssize_t nvt_gesture_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	size_t count = 0;
...
static ssize_t nvt_gesture_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	if (buf[0] == '1') {
		GESTURE_ENABLE = 1;
		hx_smwp_en_flag = 1;
		NVT_LOG("GESTURE_ENABLE Enable\n");
	} else if (buf[0] == '0') {
		GESTURE_ENABLE = 0;
		hx_smwp_en_flag = 0;
		NVT_LOG("GESTURE_ENABLE Disable\n");
	} else
		return -EINVAL;
...
static DEVICE_ATTR(gesture_enable, 0664, nvt_gesture_enable_show, nvt_gesture_enable_store);

static struct attribute *nvt_gesture_attributes[] = {
	&dev_attr_gesture_enable.attr,
	NULL
};

static struct attribute_group nvt_gesture_attribute_group = {
	.attrs = nvt_gesture_attributes
};

int nvt_sys_init(struct i2c_client *client)
{
	...
	nvt_kobject = kobject_create_and_add("gesture", NULL);
	if (!nvt_kobject) {
		NVT_LOG("create kobjetct error!\n");
		return -ENOMEM;
	}

	err = sysfs_create_group(nvt_kobject, &nvt_gesture_attribute_group);
	if (err) {
		NVT_LOG("%s() - gesture_enable node create failed: %d\n", __func__, err);
	} else {
		NVT_LOG("%s() - gesture_enable node create succeeded.\n", __func__);
	}
...
static int32_t nvt_ts_resume(struct device *dev)
{
	...
	if(0 == GESTURE_ENABLE){
		...
		NVT_LOG(" touch wakeup gesture leave\n");
	}
#else
	enable_irq(ts->client->irq);
#endif

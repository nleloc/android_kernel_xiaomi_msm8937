...
#define MODULE_FACTORY "CDFINGER"
#define CHIP_ID 0x98
char cdfinger_hardware_info[MAX_HARDWARE_INFO_LEN] = {0};
#endif
...
#define CDFINGER_REPORT_FINGER				_IOW(CDFINGER_IOCTL_MAGIC_NO,16,uint8_t)
...
#define CF_NAV_INPUT_FINGER_DOWN			606
#define CF_NAV_INPUT_FINGER_UP				607
#define CF_INPUT_FP_TAP				       608
...
static struct cdfinger_key_map maps[] = {
	...
	{ EV_KEY, CF_NAV_INPUT_FINGER_DOWN },
	{ EV_KEY, CF_NAV_INPUT_FINGER_UP },
	{ EV_KEY, CF_INPUT_FP_TAP },
};
...
#ifdef CONFIG_GET_HARDWARE_INFO
static void cdfinger_register_hardware_info(void)
{
	int ret = 0;

	snprintf(cdfinger_hardware_info, MAX_HARDWARE_INFO_LEN, "%s_0x%x",
		MODULE_FACTORY, CHIP_ID);

	ret = register_hardware_info(FINGERPRINT, cdfinger_hardware_info);
	if (0 != ret) {
		CDFINGER_ERR("register_hardware_info failed\n");
	}
}
#endif
...
static int cdfinger_parse_dts(struct device *dev, struct cdfingerfp_data *cdfinger)
{
	...
	cdfinger->pwr_num = of_get_named_gpio(dev->of_node, "cdfinger,gpio_pwr", 0);
...
static irqreturn_t cdfinger_eint_handler(int irq, void *dev_id)
{
	...
	if ((cdfingerfp->wait_finger_down == true) && (screen_status == 0)) {
			input_report_key(cdfingerfp->cdfinger_input, CF_INPUT_FP_TAP, 1);
			input_sync(cdfingerfp->cdfinger_input);
			input_report_key(cdfingerfp->cdfinger_input, CF_INPUT_FP_TAP, 0);
			input_sync(cdfingerfp->cdfinger_input);
			cdfingerfp->wait_finger_down = false;
			schedule_work(&cdfingerfp->work);
	}
...
static int cdfinger_init_irq(struct cdfingerfp_data *pdata)
{
	...
	isInit = 1;
#ifdef CONFIG_GET_HARDWARE_INFO
	cdfinger_register_hardware_info();
#endif
	return error;
}
...
static int cdfinger_report_finger(struct cdfingerfp_data *cdfinger, int arg)
{
	if(arg)
	{
		input_report_key(cdfinger->cdfinger_input, CF_NAV_INPUT_FINGER_DOWN, 1);
		input_sync(cdfinger->cdfinger_input);
		input_report_key(cdfinger->cdfinger_input, CF_NAV_INPUT_FINGER_DOWN, 0);
		input_sync(cdfinger->cdfinger_input);
	}
	else{
		input_report_key(cdfinger->cdfinger_input, CF_NAV_INPUT_FINGER_UP, 1);
		input_sync(cdfinger->cdfinger_input);
		input_report_key(cdfinger->cdfinger_input, CF_NAV_INPUT_FINGER_UP, 0);
		input_sync(cdfinger->cdfinger_input);
	}
...
static long cdfinger_ioctl(struct file* filp, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case CDFINGER_REPORT_FINGER:
			err = cdfinger_report_finger(cdfinger, arg);
			break;

		case CDFINGER_GETID:
#ifdef SUPPORT_ID_NUM
			err = cdfinger->chip_id;
			if(cdfinger->chip_id != 0){
				mem_class = class_create(THIS_MODULE, "fingerprint");
				if (mem_class==NULL)
				{
					CDFINGER_ERR("create cdfinger class failed!\n");
					err = (-EFAULT);
				}
			}
#endif
										...
										unsigned long event, void* data)
{
	...
	switch (blank) {
		...
		case FB_BLANK_POWERDOWN:
			mutex_lock(&g_cdfingerfp_data->buf_lock);
			screen_status = 0;
			g_cdfingerfp_data->wait_finger_down = true;
			if (isInKeyMode == 0)
				cdfinger_async_report();
			mutex_unlock(&g_cdfingerfp_data->buf_lock);
			break;
...
static int cdfinger_probe(struct platform_device *pdev)
{
	...
	if (status != 0) {  // id pin is low , cdfinger
	...
	cdfingerdev->wait_finger_down = false;
	INIT_WORK(&cdfingerdev->work, notification_work);
